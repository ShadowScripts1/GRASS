...
# Manual input for the waiting time in minutes
try:
    wait_time_minutes = int(input("Proxy Rotation in minutes: "))
except ValueError:
    logger.error(f"{Fore.RED}Invalid input. Using the default timeout of 120 minutes")
...
async def connect_to_wss(socks5_proxy, user_id):
    ...
    logger.info(f"{Fore.GREEN}Device connection established")
...
async def main():
    ...
    for user_id in user_ids:
        ...
        #     logger.info(f"{Fore.YELLOW}Not enough proxies remaining to use.")
        #     break

        # Assign all available proxies for this user
        proxies_for_user = auto_proxy_list  # Use all available proxies

        # Combine new proxies with the existing ones
        user_proxy_file = f"{user_id}_proxies.txt"
        try:
            with open(user_proxy_file, 'r') as file:
        ...
        # Combine new proxies with existing ones
        combined_proxies = list(set(existing_proxies + proxies_for_user))

        # Write the combined proxies to the file
        with open(user_proxy_file, 'w') as file:
       ...
       # logger.info(f"{Fore.GREEN}Moving to the next account after trying all proxies for user_id: {user_id}")
